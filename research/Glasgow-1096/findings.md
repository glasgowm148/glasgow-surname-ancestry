# Findings: [Robert Glasgow (Glasgow-1096), born 1520, Glasgow, Lanarkshire, Scotland](https://www.wikitree.com/wiki/Glasgow-1096)

Record every useful result and negative search here as it is found.
Do not treat an unsourced tree or naming pattern as proof.

## Current conclusion

Father assessment: [William Glasgow (1500)](https://www.wikitree.com/wiki/Glasgow-1098) is **probable on strong circumstantial evidence**, not proved by an explicit kinship formula.

This is stronger than a bare chronology-and-surname placement. William's proved
son [[Glasgow-1093|John Glasgow]] and Robert repeatedly occupied the same
compact south-burgh property network in 1557-73. Their lands bounded the same
tenements in 1557 and 1560, and a John Glasgow, fisher, held the property next
to Robert and witnessed Robert and Margaret Rowand's transaction in 1572/73.
Robert and John were then described together as fishers in 1573. Robert had
already witnessed [[Glasgow-1097|Archibald Glasgow]]'s 1551 purchase, whose
exact east-side parcel was in John's hands by 1554/55. The combined pattern is
that of one close surname, craft and property group, not three isolated men.

The 1573 fisher entry also names **Archibald Herbertsone** between Robert and
John. Archibald is the record-proved husband of [[Glasgow-3916|Isobella
Glasgow]], who is independently a probable daughter of William because her
husband witnessed John's disposal of William's inherited parcel in 1564. This
creates a repeated Robert–John–Isobella/Archibald cluster rather than a merely
bilateral Robert–John association, materially strengthening the proposed
sibling group while still supplying no explicit kin term.

Male primogeniture also explains why only John was later served heir to
William's inherited tenement: that inheritance does not exclude younger sons
who established neighbouring holdings. The competing explanations remain that
the fisher John was Robert's own son, or that Robert was a cousin or unrelated
namesake. Because two generations of adult Johns overlapped, the evidence does
not justify a `certain` WikiTree status. It does justify retaining William as
Robert's uncertain father while describing the relationship as **probable**.

Robert is directly proved to have had **Ninian as a son**. The 1596 burgess
admission calls [Ninian Glasgow (Glasgow-3188)](https://www.wikitree.com/wiki/Glasgow-3188)
Robert's **second son**. Ninian's 1613 testament independently names
"Johne Glasgw his brother." That proves Ninian had a brother John alive on 10
January 1613, but does not say whether John was a full or half-brother. Robert
is therefore a strong father candidate for John, not a directly proved father;
nor does *second son* establish whether John was older or younger than Ninian.

[Margaret Rolland (Rolland-239), born about 1535, Glasgow](https://www.wikitree.com/wiki/Rolland-239)
is Robert's **confirmed spouse**. Protocols 1903, 1904 and 2125 call the couple
spouses. Protocol 2002 identifies Robert among the fishers, matching the 1596
occupation and resolving the property-holder to this profile. Robert was alive
on 7 February 1574/75, so the live `after 1572` death boundary is obsolete.

[Elizabeth Glasgow, widow of Alexander Kaine](https://www.wikitree.com/wiki/Glasgow-3915)
is a new **possible daughter**, not a proved child. Ninian's sons
[Robert](https://www.wikitree.com/wiki/Glasgow-1027) and
[William](https://www.wikitree.com/wiki/Glasgow-3075) each have a daughter whose
name is recorded as Elizabeth/Elizabethe or Bessie. This independent repetition
makes Elizabeth worth testing as Ninian's missing older sister. It remains only
a naming-pattern hypothesis: Elizabeth's current `before 1530` birth is no more
than ten years after Robert's approximate 1520 birth, and no source links the
Easter Corswood widow to Robert's Glasgow household.

## Source findings

| Source | Finding | Assessment |
| --- | --- | --- |
| Civic and property evidence summarized in the [captured WikiTree profile](https://www.wikitree.com/wiki/Glasgow-1096) | Chronology, adjacent burgh property and Robert's association with Archibald fit William's family, but no record names Robert's father. | Indirect cluster evidence. |
| Robert Renwick, ed., *Abstracts of Protocols of the Town Clerks of Glasgow*, vol. 1, protocol 84, [21 May 1551, printed p. 31](https://archive.org/details/abstractsprotoc07robegoog/page/n59/mode/2up) | Robert Glasgow witnessed the conveyance of a Glasgow tenement to Archibald Glasgow and Isobell Newlands. | Earliest located civic appearance. It directly places Robert with Archibald but states no kinship. |
| Same work, vol. 1, protocols 183-184, [10 May 1554, printed p. 63](https://archive.org/details/abstractsprotoc07robegoog/page/n90/mode/2up) | Robert Glesgw and David Mathy were named together among the `comburgesses` witnessing John Androsoune's sasine and subsequent conveyance to himself and his spouse. | Direct contemporary evidence that Robert was a fellow burgess and personally associated with David Mathy by 1554. David's documented son John is a plausible route into the later Glencairn Mathie household, whose possible descendant Jonet married John Glasgow in 1618. See the [focused analysis](../Glasgow-3908/mathie-glencairn-bridge-1554-1618.md). |
| Same work, vol. 2, protocol 266, [17 February 1555/56, printed pp. 2-3](https://archive.org/details/abstractsprotoc08scogoog/page/n18/mode/2up) | Robert Glesgw, expressly described as a burgess, witnessed John Mychell and Cristine Bogile's property transaction. | Independent confirmation of burgess status. |
| Same work, vol. 2, protocols 372-373, [26 July 1557, printed p. 35](https://archive.org/details/abstractsprotoc08scogoog/page/n51/mode/2up) | Robert witnessed William Hall's sasine in a South Port-to-Clyde-bridge tenement and its immediate sale to Marion Smyth. | Direct witness occurrence in the same south-burgh district as Robert's later property. |
| Same work, vol. 2, protocols 401 and 403, [8 and 26 November 1557, printed pp. 41-42](https://archive.org/details/abstractsprotoc08scogoog/page/n57/mode/2up) | A tenement lay between Robert Glesgw's lands on the south and John Glesgw's lands on the west. | Strong evidence that Robert and John held adjoining property; the protocols state no relationship. |
| Same work, vol. 2, protocols 525-526, [20 February 1559/60, printed p. 76](https://archive.org/details/abstractsprotoc08scogoog/page/n93/mode/2up) | Robert Glesgw witnessed a South Port-to-Clyde-bridge property sale to David Howe and Cristina Lyndesay and its immediate resale. | Direct witness occurrence continuing Robert's association with the district. |
| Same work, vol. 2, protocols 544-546, [18 April 1560, printed pp. 80-81](https://archive.org/details/abstractsprotoc08scogoog/page/n98/mode/2up) | Three sasines used Robert Glesgw's lands as a boundary; protocol 544 also used John Glesgw's lands as the western boundary. | Direct property evidence. The compact sequence supports, but does not prove, the identification of the neighbouring John with Glasgow-1093. |
| James R. Anderson, ed., ''Burgesses & Guild Brethren of Glasgow, 1573-1750'' (1925), [p. 24](https://archive.org/details/scottishrecordso43scotuoft/page/24/mode/2up), 23 October 1596 | Ninian Glasgow was admitted as "second son to Robert G., fisher, burgess." | Directly proves Robert was Ninian's father and confirms Robert's occupation and burgess status. |
| NRS Glasgow Commissary Court, `CC9/7/9`, images 142-145, testament of Ninian Glasgow, made 10 January 1613 and confirmed 8 May 1613; [saved source extracts](sources/robert-sons-source-extracts.txt) | Ninian directly names `Johne glasgw his brother` and `Marioun glasgw his sist[er]`. The exact clothing description for John is unclear. | Directly proves Ninian had siblings John and Marion alive in 1613. Because the will does not say whether either was a full sibling, Robert is a strong parent candidate rather than a directly proved parent for them; `second son` does not identify John's birth order. |
| Robert Renwick, ed., *Abstracts of Protocols of the Town Clerks of Glasgow*, vol. 6, protocols 1903-1904, [12 February 1572/73](https://archive.org/details/abstractsprotoc01renwgoog/page/n121/mode/2up) | Robert Glasgow and Margaret Rowand dealt with a tenement adjoining land of John Glasgow; John Glasgow, fisher, witnessed the transaction. | Directly places Robert and a fisher John in the same immediate property network. It does not call John Robert's son. |
| *Extracts from the Records of the Burgh of Glasgow*, accounts for 1573-74, printed p. 453, [FamilySearch image](https://www.familysearch.org/ark:/61903/3:1:3QHV-V3DN-P9VH-Q) | The burgh accountant was discharged for eight shillings described as outstanding and not in use of payment from "Robert Glasgow tenement." | New contemporary property occurrence. Its date and subject fit the same Robert who dealt with a Glasgow tenement in 1572/73 and 1574/75. The entry gives no occupation or relative, so identity with Glasgow-1096 is strong contextual attribution rather than a uniquely identifying statement. |
| Robert Renwick, ed., *Abstracts of Protocols of the Town Clerks of Glasgow*, vol. 7, protocol 2002, [19 December 1573](https://archive.org/details/abstractsprotoc00robegoog/page/n52/mode/2up) | `Robert Glasgw, Archibald Herbertsone, John Glasgw, fishers` were named together as witnesses. Archibald is the proved husband of probable sister Isobella Glasgow. | Strong three-person occupational and association evidence for the proposed sibling/in-law cluster. The protocol states no kinship, and multiple adult Johns were present, so it cannot by itself resolve which John witnessed. |
| Robert Renwick, ed., *Abstracts of Protocols of the Town Clerks of Glasgow*, vol. 7, protocol 2125, [7 February 1574/75, printed p. 86](https://archive.org/details/abstractsprotoc00robegoog/page/n102/mode/2up) | Robert Glasgow and Margaret Rowand are expressly called spouses in a Glasgow property reversion. | Direct proof of Robert's spouse and of his presence as a property holder in Glasgow; the record does not name their children or prove Margaret was their mother. The attached surname `Rolland` should be corrected to `Rowand` if it represents this woman. |

## WikiTree update instructions

Live profile refreshed **19 July 2026**. [[Glasgow-3188|Ninian Glasgow]] remains
attached as Robert's certain son. [[Glasgow-3181|John Glasgow of
Hirmanscheillis]] is now also attached and his biography incorrectly applies
the 1613 brother clause to him. That John died in 1576 and cannot be the brother
named in Ninian's testament of 10 January 1613. This is a demonstrated identity
conflict, not merely a weak attachment.

The remaining changes are:

- Add that the 1596 roll calls Ninian Robert's second son and directly describes
  Robert as a fisher and burgess, with the cited page link above.
- Add protocol 2125 as direct evidence for spouse Margaret **Rowand** and correct
  the existing Rolland-239 profile if it represents this woman.
- Change Robert's uncertain death boundary to **after 7 February 1575**.
- Do not create or merge a replacement John yet. First exclude the existing
  candidates, especially Glasgow-1030, by checking their spouses and children.
- In that John's research notes, add that a fisher John Glasgow held adjoining
  property and witnessed with Robert in 1572-73. Treat him only as one possible
  identity candidate: multiple adult Johns were present and the protocols state
  no father-son relationship.
- Remove Glasgow-3181 as Robert's son; his 1576 death excludes that identity.
- Do not infer the identity of Robert's first son, or assign brother John to
  Robert, solely from Ninian being called the second son.

### Exact relationship reconstruction

| Relationship | WikiTree profile | Required treatment |
| --- | --- | --- |
| Probable father | [[Glasgow-1098|William Glasgow (about 1500), Glasgow burgh]] | Retain the structured relationship as uncertain, but describe it as probable: Robert repeatedly shared William's proved son's compact property/craft network and linked the same group to Archibald. No record explicitly names Robert's father. |
| Spouse | [[Rolland-239|Margaret Rolland]] | Protocol 2125 directly names Robert's spouse as **Margaret Rowand**. Correct this existing profile's surname if it represents the same woman; the protocol does not prove she was mother of either son. |
| Proved son | [[Glasgow-3188|Ninian Glasgow (about 1550), Glasgow]] | Already attached certain; the 1596 roll directly calls him Robert's second son. |
| Strong candidate son | **John Glasgow, alive 10 January 1613; unresolved identity within an existing profile** | Ninian's testament directly calls John his brother. Robert is Ninian's proved father, but full versus half-brother is unstated. [John Glasgow (Glasgow-1030), born before about 1595, Glasgow](https://www.wikitree.com/wiki/Glasgow-1030) is the leading identity candidate; do not treat either the identity or Robert father edge as certain. |
| Strong candidate daughter | [[Glasgow-3062|Marion Glasgow]] | Ninian's testament directly calls Marion his sister. Robert is Ninian's proved father, but the record does not establish whether Marion was a full or half-sister. Replace the impossible Ninian-as-father edge with Robert marked uncertain; do not infer Marion's mother. |
| Other current child | [[Glasgow-3613|Margaret Glasgow]] | Existing attachment; the 1596 record does not address her. |
| Possible daughter | [Elizabeth Glasgow, widow of Alexander Kaine](https://www.wikitree.com/wiki/Glasgow-3915) | Collateral descendant-name lead only. Do not attach without a Kaine, property or testamentary record linking her to Robert or Ninian. |

Do **not** merge the proved John into
[[Glasgow-1030|John Glasgow (about 1575), Glasgow]] or
[[Glasgow-1561|John Glasgow (about 1579), Linlithgowshire]] yet. Both were alive
in the relevant period, but neither marriage or baptism record names a father.
[[Glasgow-3181|John Glasgow of Hirmanscheillis]] is excluded because he died in
1576, thirty-seven years before Ninian's bequest.

Edit summary: `Add Robert's directly evidenced spouse and 1575 occurrence; remove the misidentified 1576 son.`

## Kilbride identity candidate - 31 August 2026

[Robert Glasgow of Kilbride (Glasgow-3919)](https://www.wikitree.com/wiki/Glasgow-3919)
is now the leading later identity for this Robert. The five-year interval from
Robert and Margaret Rowand's 1575 city act to the 1580 Kilbride charter is
compatible. Robert's 1554 co-witness David Mathy belongs to a documented route
into the Glencairn estate landscape, while Kilbride co-occupants include
Dunlop, Wilson and Young surnames also recurring in Robert's burgh/property
network.

Assessment: **probable same man, moderate-to-strong circumstantial**. The 1580
record supplies no occupation, spouse or city designation, so retain separate
profiles and an identity link rather than merge.
